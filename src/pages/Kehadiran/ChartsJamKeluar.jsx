...
export function ChartsJamKeluar() {
  const [chartData, setChartData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get("/jamkeluar");
        ...
        data.forEach((entry) => {
          const entryDate = new Date(entry.jamkeluar).toLocaleDateString(
            "id-ID"
          );
          if (entryDate === today) {
            let keterangan;
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader>
        <CardTitle>Presensi Jam Keluar</CardTitle>
      </CardHeader>
      <CardContent>
        <ScrollArea>
          <ChartContainer
